...
pub trait NodeUpdater {
    fn update(
        &mut self,
        node: (NodeId, &mut Box<dyn RuntimeNode>),
...
//     fn update(
//         &mut self,
//         node: (NodeId, Box<dyn RuntimeNode>),
//         node_description: Option<NodeDescription>,
//     ) {
...
impl NodeUpdater for SingleThreadedNodeUpdater {
    fn update(
        ...
        //if let Ok(mut n) = node.1. {
        //let mut n = node.1.clone();
        let _update_span = if let Some(ref desc) = node_description {
            ...
            info_span!("st_on_update", node.id = node.0.to_string().as_str())
        }
        .entered();
        let started = std::time::Instant::now();
        let result = node.1.on_update();
