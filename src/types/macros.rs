            ...
            pub fn [<connect_nodes_ $type>](
                ...
                if let Some(sender_output) = sender_io.downcast_mut::<TypedOutput<$type>>() {
                    if let Some(receiver_input) = receiver_io.downcast_mut::<TypedInput<$type>>() {
                        ...
                        println!("[generate_local_connection] Successfully connected nodes {} -> {} with type {:?}", sender_id, receiver_id, stringify!($type));
                    } else {
                ...
                println!("[generate_local_connection] Local nodes connected: {} -> {} (Out {} -> In {})", sender_id, receiver_id, sender_out_idx, recv_in_idx);
            }

            // Register the connection function in the type registry
            lazy_static::lazy_static! {
                static ref REGISTER: () = {
                    TYPE_REGISTRY.lock().unwrap().register::<$type>([<connect_nodes_ $type>]);
                };
            }
        }
    };
